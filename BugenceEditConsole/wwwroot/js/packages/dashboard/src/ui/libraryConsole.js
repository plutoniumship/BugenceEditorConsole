...
function normalizePageItem(raw) {
    ...
    const publishedIso = publishedSource === null ||
...
function ensureIsoString(value, fallback) {
    ...
    return (Number.isNaN(parsedFallback.getTime()) ? new Date() : parsedFallback).toISOString();
}
...
function ensureSlug(candidate, name) {
...
function ensureId(candidate, slug, name) {
