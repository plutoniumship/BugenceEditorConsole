import { createEventBus, createStore, fetchHistory, fetchPageCollection } from "@bugence/core";
...
const syncBus = createEventBus(SYNC_CHANNEL_NAME, {
...
function withTimelineState(pageId, updater) {
...
function getTimelineStateSnapshot(pageId) {
    return store.getState().timelineByPage[pageId] ?? createTimelineState();
}
function pushTimelineEvent(pageId, event) {
...
function setDirtySectionState(pageId, sectionId, nextState) {
    withTimelineState(pageId, (current) => {
...
function addConflictSection(pageId, sectionId) {
    withTimelineState(pageId, (current) => {
...
function clearConflictSection(pageId, sectionId) {
    withTimelineState(pageId, (current) => {
        if (!current.conflictSectionIds.includes(sectionId)) {
            return current;
        }
...
function setReviewStatus(pageId, sectionId, status) {
    withTimelineState(pageId, (current) => {
...
function recordPublishSummary(pageId, summary) {
    withTimelineState(pageId, (current) => ({
        ...current,
        publishSummary: summary
    }));
}
function recordSyncTelemetry(pageId, telemetry, syncedAtUtc) {
    withTimelineState(pageId, (current) => ({
        ...current,
        lastSyncTelemetry: telemetry,
        lastSyncedAtUtc: telemetry.result === "error"
            ? current.lastSyncedAtUtc
            : syncedAtUtc ?? new Date().toISOString()
    }));
}
function appendHistoryEntriesToTimeline(pageId, entries) {
    if (!entries.length) {
        return;
    }
    withTimelineState(pageId, (current) => {
...
function cleanSectionKey(diff, fallback) {
    return (diff?.after?.payload.sectionKey ??
        diff?.before?.payload.sectionKey ??
        fallback);
}
function resolveSectionLabel(pageId, sectionId, diff) {
    ...
    if (timeline) {
        ...
        const summaryEntry = timeline.publishSummary?.entries.find((entry) => entry.sectionId === sectionId);
...
function describeDiff(diff) {
    ...
    const contentType = diff.after?.payload.contentType ?? diff.before?.payload.contentType;
...
function describeReviewStatus(status) {
    switch (status) {
        case "approved":
...
function buildHistoryEvent(entry) {
...
function handleSyncEvent(envelope) {
    ...
    switch (topic) {
        ...
        case "section.changed": {
            ...
            const previousDirty = Boolean(getTimelineStateSnapshot(pageId).dirtySections[sectionId]);
            if (data.dirty) {
...
export function selectDashboardPage(pageId) {
    store.setState((current) => ({
        ...current,
...
export function getSelectedDashboardPage() {
    const state = store.getState();
...
export async function loadDashboardHistory(options = {}) {
    const { pageId, take, force = false, ttlMs } = options;
    const key = `${pageId ?? "all"}:${take ?? "default"}`;
    const state = store.getState();
    ...
    try {
        const result = await fetchHistory({ pageId, take }, { revalidate: force, ttlMs });
...
export function getDashboardHistory(pageId, take) {
    const key = `${pageId ?? "all"}:${take ?? "default"}`;
    return store.getState().history[key]?.items ?? [];
}
export function getTimelineSnapshot(pageId) {
...
async function fetchHistoryForSync(pageId) {
    const key = `${pageId}:default`;
    const state = store.getState();
    const cached = state.history[key];
    const result = await fetchHistory({ pageId }, { revalidate: true, ttlMs: 0 });
    const historyItems = result.data.history ?? [];
    const etag = result.etag ?? result.data.etag ?? cached?.etag;
    store.setState((current) => ({
        ...current,
...
export function startDashboardSync(options) {
    if (!isBrowser()) {
        return;
    }
