import { sidebarCards, workflowSchemas, getSidebarCard, getWorkflow, resolveMetrics } from "@bugence/core";
const METRIC_IDS = [
    "draftCount",
    "lastPublishedAtUtc",
    "accessibilityScore",
...
const CARD_SEQUENCE = ["publishing_overview", "quality_checks", "quick_actions"];
export function initSchemaSidebar() {
    const root = document.querySelector("[data-schema-sidebar]");
    if (!root) {
        return;
    }
    const cardsHost = root.querySelector("[data-schema-cards]") ?? root;
    const workflowHost = root.querySelector("[data-schema-workflow]");
...
function renderCard(schema, metrics) {
  ...
  `;
    ...
    if (schema?.metrics?.length) {
        ...
        schema.metrics.forEach((metric) => {
            ...
            metricGrid.appendChild(renderMetric(metric.label, displayValue, metric.description, metric.icon, metric.emphasis));
        });
...
function collectMetricSource(root) {
    return {
...
async function resolveSidebarMetricValues(pageId, source) {
    try {
...
function buildFallbackMetricMap(source) {
    return METRIC_IDS.reduce((acc, key) => {
        acc[key] = { raw: source[key] };
        return acc;
    }, {});
}
function renderMetric(label, value, description, icon, emphasis = "default") {
