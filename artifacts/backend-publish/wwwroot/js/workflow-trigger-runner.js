(function () {
  ...
  const recentSubmitTriggerByForm = new WeakMap();
  ...
  const collectFields = (form) => {
    ...
    // Canonical aliases for workflow email templates
    const details = firstFilled('details', 'message', 'inquiry', 'comment', 'description', 'body', 'text');
    if (details) {
      if (!fields.details) fields.details = details;
      if (!fields.message) fields.message = details;
    }

    const fullName = firstFilled('fullName', 'fullname', 'full_name', 'name');
    if (fullName) {
      if (!fields.fullName) fields.fullName = fullName;
      if (!fields.name) fields.name = fullName;
    }
  ...
  const attach = () => {
      ...
      el.addEventListener('click', () => {
        ...
        if (form) {
          ...
          if (tag === 'button' && (type === '' || type === 'submit')) {
            ...
            if (el.hasAttribute('data-bugence-workflow-id') || el.hasAttribute('data-bugence-workflow-dguid')) {
              recentSubmitTriggerByForm.set(form, el);
            }
            return;
          }
    ...
    document.querySelectorAll('form').forEach((form) => {
      ...
      form.addEventListener('submit', (event) => {
        ...
        const recentTrigger = recentSubmitTriggerByForm.get(form);
        if (isRecentSubmitClick && recentTrigger && (recentTrigger.hasAttribute('data-bugence-workflow-id') || recentTrigger.hasAttribute('data-bugence-workflow-dguid'))) {
          event.preventDefault();
          trigger(recentTrigger, 'submit');
          return;
        }
