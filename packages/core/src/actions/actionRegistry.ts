...
function assertRegistration(registration: ActionRegistration): void {
...
function isCapabilitySatisfied(meta: ActionMeta): boolean {
...
async function resolveModule(entry: RegisteredAction): Promise<ActionModule> {
  ...
  if (!loaded || typeof loaded.execute !== "function") {
    throw new Error(
      `Action '${entry.registration.meta.id}' did not return a module with an execute function.`
    );
  }
...
export function registerActions(registrations: ActionRegistration[]): void {
...
export function getActionMeta(id: string): ActionMeta | null {
  const entry = actions.get(normaliseId(id));
  return entry ? entry.registration.meta : null;
}

export function listActionMetas(filter?: (meta: ActionMeta) => boolean): ActionMeta[] {
...
export async function loadActionModule(id: string): Promise<ActionModule> {
  const entry = actions.get(normaliseId(id));
  if (!entry) {
...
export async function executeAction<TParams = unknown, TContext extends ActionContext = ActionContext>(
  id: string,
  context: TContext,
  params?: TParams
